...
main(int argc, char* argv[])
{
  ...
  for(int n=1; n<=nramp;n++){
   printf ("/n/n---Start ramp fitting using %d samples--------------------\n",n);
 
   //int ndata =0;
   //int nwork = n+1;

   //while(--nwork){ //n>=2 de n<2 wo calculate 
   //for(int nn=1; nn<=n;nn++){  
   //printf ("nwork= %d\n",nwork);
   //printf ("Seeing fits file no.%d\n",nn);
   printf ("Seeing fits file no.%d\n",n);
   //if (nwork==1) load_image(work, filename1);
   if (n==1) load_image(work, filename1);
   //if (nwork==2) load_image(work, filename2);
   if (n==2) load_image(work, filename2);
   //if (nwork==3) load_image(work, filename3);
...
int test4=0;
while(--n)LOOP{
   printf ("LOOP2 test4= %d, M= %d, P= %d, Q= %d\n",test4, M,P,Q);    
